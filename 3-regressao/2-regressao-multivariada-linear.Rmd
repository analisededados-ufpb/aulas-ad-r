...
advertising %>% 
  select(-campaign) %>% 
  ggpairs(lower = list(continuous = wrap("points", alpha = 0.3)), progress = FALSE)
```
...
Caso apliquemos o modelo em todos os pontos de `advertising`, teremos várias estimativas para um mesmo valor de `Radio`, já que existem várias combinações de um dado valor dessa variável com as demais variáveis de entrada no modelo. Isso fica confuso muito fácil. 
...
```{r}
multi = lm(Sales2 ~ TV2 + Radio + Newspaper + Radio*TV2 + Radio*Newspaper, data = advertising)
...
Qual o intervalo de confiança da estimativa de `Sales2` para uma certa combinação de valores de investimento em Rádio, TV e Jornal impresso?

```{r}
predict(multi, 
        data.frame(Radio = 10e3, TV2 = log2(20e3), Newspaper = 0), 
        interval = "predict")
